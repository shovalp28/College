...
{
    public class CollegeLoginSController : Controller
    {
        public static Hashtable _validationCodesHashTable = new Hashtable();

        // GET: CollegeLoginS
        public ActionResult OpenStudentsLoginPage()
        {
            ...
            return View("CollegeLoginS", logInScreenData);
        }
        public ActionResult OpenStudentsValidationPage(string id)
        {
            ...
            return View("CollegeLoginSVald", logInScreenData);
        }
        public ActionResult BtnValidationloginClicked(string inputVcode, string mobileValidationCode, string id )
        {
            ...
            return RedirectToAction("StudentLessonList", "StudentTimeTable", new { id = id });
        }
        ...
        public ActionResult BtnloginClicked(string id)
        {
            ...
            if (WS.StudentIdNumberExist(sesId, id))
            {
                if (string.IsNullOrEmpty(WS.GetStudentMobileById(sesId, id)))
                {
                    return JavaScript("window.alert('. לא הוגדר טלפון נייד בכרטיס שלך, יש לפנות למכללה לטיפול');");
                }
                ...
                var mobileNumber = WS.GetStudentMobileById(sesId,id);
               ...
               if(_validationCodesHashTable.ContainsKey(id))
                ...
                return RedirectToAction( "OpenStudentsValidationPage", "CollegeLoginS", new { id=id}); // redirects to internal url
